...
/* 
        ...
        chart.getXYPlot().setRangeAxis(R1, axis);
        chart.getXYPlot().setRangeAxisLocation(R1, AxisLocation.BOTTOM_OR_LEFT);
 */
        ...
        chart1.getXYPlot().getRenderer(R1).setToolTipGenerator(new XYToolTipGenerator(){
            ...
            public String generateToolTip(XYDataset ds, int s, int i){
                ...
                if(N==3) {
                    ...
                    // infusione
                    str="Rest di "+((int)(d2-d1))+" minuti a "+(int)y1+"°C,\npartendo da "+(int)y0+"°C, in "+((int)(d1-d0))+" minuti";
                } else if(N==6){
                    // decozione
                    double y2=ds.getYValue(s,2);
                    double d2=ds.getXValue(s,2);
                    ...
                    str+="Decozione del ";
                    str+=" "+(int)F+"% dell'impasto, prima per "+(int)(d2-d1)+" minuti a "+(int)y2+" °C";
    ...
    private void initComponents() {
        ...
        jComboBox1.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "", "Alluminio", "Acciaio", "Plastica" }));
    ...
    private void btnRem5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRem5ActionPerformed
        if(!tblMaltSteps2.isEnabled())return;
    ...
    public void ricalcolaInfusioni(){
        ...
        List<MashStep> ls=(List<MashStep>)new LinkedList<MashStep>(this.mashDecoctionStepTableModel.getRows()).clone();
        Collections.sort(ls);
        ...
        for(MashStep y: ls){
            if(flag){
                ...
                first.setLength(
                        y.getStart()+y.getDecoctionRamp1()+y.getDecoctionRamp2()+y.getDecoctionRestL1()+y.getDecoctionRestL2()+
            ...
            if(prec!=first){
                int start=0;
                if(yPrec!=null) {
                    start=y.getStart()-(yPrec.getStart()
                    +yPrec.getDecoctionRamp1()+yPrec.getDecoctionRamp2()+yPrec.getDecoctionRestL1()+yPrec.getDecoctionRestL2()
                    +yPrec.getDecoctionRampDown())
                    ;
                }
                prec.setLength(
                        start+
                        y.getDecoctionRamp1()+y.getDecoctionRamp2()+y.getDecoctionRestL1()+y.getDecoctionRestL2()
    ...
    public void ricalcolaInfusioniDaAggiunte(){
        ...
        @SuppressWarnings("unchecked")
        List<MashStep> ls=(List<MashStep>)new LinkedList<MashStep>(this.mashInfusionStepTableModel.getRows()).clone();
        Collections.sort(ls);
    ...
    private void btnRem4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRem4ActionPerformed
        if(!tblMaltSteps1.isEnabled())return;
    ...
    public String getDesc(){
    	//ixtlanas per desc forum (append a getDes4Forum di recipedata) SOLO MASH STEPS NO DECOZIONE
        String str="";
        if(this.mashStepTableModel.getRows().size()<1) return str;
        str+="Mash Steps:\n";
...
//        }
    ...
    private void btnRem3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRem3ActionPerformed
        if(!tblMaltSteps.isEnabled())return;
